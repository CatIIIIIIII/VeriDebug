...
from transformers.integrations.deepspeed import (
    deepspeed_init, deepspeed_load_checkpoint, is_deepspeed_available
)
...
class GradCacheTrainer(Trainer):
    def create_accelerator_and_postprocess(self):
        ...
        kwargs = InitProcessGroupKwargs(
            timeout=timedelta(seconds=36000))  # 10 hours
    ...
    def _inner_training_loop(
        self, batch_size=None, args=None, resume_from_checkpoint=None, trial=None, ignore_keys_for_eval=False
    ):
        ...
        if not self.no_emb_gas:
            ...
            if os.getenv("BF16", False):
                gc = GradCache(
                    ...
                    get_rep_fn=lambda x: x["q_reps"].to(
                        dtype=dtype) if dtype is not None else x["q_reps"],
                )
            else:
                gc = GradCache(
                    models=[model, model, model],
                ...
                with self.accelerator.accumulate(model):
                    ...
                    if self.mode in ["unified", "generative"]:
                        ...
                        if self.no_gen_gas:
                            ...
                            chunks = gc.split_inputs(
                                inputs["generative"], self.gc_chunk_size)
                            for chunk in chunks:
                                loss_gen_chunk = model(
                                    generative=chunk).loss_gen / len(chunks)
                    ...
                    if self.mode in ["unified", "embedding"]:
                        ...
                        if self.split_emb:
                            ...
                            assert torch.allclose(
                                loss_emb_q, loss_emb_p), f"{loss_emb_q} != {loss_emb_p}"
                            loss_emb = loss_emb_q

                        elif self.split_emb_full:
                            with self.compute_loss_context_manager():
                                out = model(
                                    query=inputs["query"], passage=inputs["passage"], q_grad=False, pos_grad=False)
                            ...
                            with self.compute_loss_context_manager():
                                loss = model(
                                    q_reps=q_reps, passage=inputs["passage"], p_reps=p_reps, q_grad=False, neg_grad=False).loss
                            ...
                            with self.compute_loss_context_manager():
                                loss = model(
                                    query=inputs["query"], p_reps=p_reps, pos_grad=False, neg_grad=False).loss
...
#                        import pdb; pdb.set_trace()
                    ...
                    if self.mode == 'unified':
                        ...
                        self.state.loss_emb = getattr(
                            self.state, "loss_emb", torch.tensor(
                                0.0).to(loss_emb.device)
                        )
                        self.state.loss_gen = getattr(
                            self.state, "loss_gen", torch.tensor(
                                0.0).to(loss_emb.device)
                        )
