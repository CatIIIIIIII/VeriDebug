    ...
    def __init__(self, temperature: float, negatives_cross_device: bool):
        ...
        if self.negatives_cross_device:
            if not dist.is_initialized():
                raise ValueError(
                    'Cannot do negatives_cross_device without distributed training')
...
class GritLMTrainModel(GritLM):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.emb_loss_fn = DistributedContrastiveLoss(
            temperature, negatives_cross_device)
    ...
    def encode(self, features):
        ...
        out = (getattr(self.model, self.embedding_attr)
               if self.embedding_attr else self.model)(**kwargs)[0]
        ...
        if instruction_lens is not None:
            ...
            for i, l in enumerate(instruction_lens):
                ...
                assert attention_mask[i].sum(
                ) > 0, f"All 0: {attention_mask[i]}, l: {l}"
    ...
    def forward(
        ...
        bug_type: Dict[str, torch.Tensor] = None,
        ...
        if generative is not None:
            if self.gen_loss_fn is not None:
                ...
                loss_gen = self.gen_loss_fn(
                    generative.pop('labels'), self.model(
                        **generative, **self.gen_add_kwargs).logits
                )
            else:
        ...
        if self.do_classify:
            ...
            print(bug_type)
            exit()
            cls_loss = self.cls_loss_fn(q_reps, bug_type)
