        ...
        if self.mode in ["unified", "embedding"]:
            ...
            if len(self.ds_embedding[item]['neg']) < self.args.train_group_size - 1:
                num = math.ceil((self.args.train_group_size - 1) /
                                len(self.ds_embedding[item]['neg']))
                negs = random.sample(
                    self.ds_embedding[item]['neg'] * num, self.args.train_group_size - 1)
            else:
                negs = random.sample(
                    self.ds_embedding[item]['neg'], self.args.train_group_size - 1)
    ...
    def __call__(self, features):
        ...
        if isinstance(query[0], (tuple, list)):
            ...
            query = [
                self.base_bos + self.user_bos +
                f[0].strip("\t\n :") + self.user_eos +
                self.embed_bos + f[1] + self.embed_eos
                ...
                [
                    len(
                        self.tokenizer.tokenize(
                            self.user_bos + z + self.user_eos + self.assistant_bos)
                        if i > 0 else
                        self.tokenizer.tokenize(
                            self.base_bos + self.user_bos + z + self.user_eos + self.assistant_bos)
                    )
                    if i % 2 == 0 else
        ...
        if generative[0] is not None:
            ...
            features["generative"]["labels"][:, 1:][features["generative"]
                                                    ["labels"][:, 1:] == self.tokenizer.pad_token_id] = -100

        if q_instruction_lens:
            # Check that there is no mistake
            for i, l in enumerate(q_instruction_lens):
                assert features["query"]["input_ids"][i,
                                                      l] != self.tokenizer.pad_token, f"No text to embed: {query[i]}"
            for i, l in enumerate(d_instruction_lens):
                assert features["passage"]["input_ids"][i,
                                                        l] != self.tokenizer.pad_token, f"No text to embed: {passage[i]}"
            # Need to be masked out later
            features["query"]["instruction_lens"] = torch.tensor(
                q_instruction_lens)
            features["passage"]["instruction_lens"] = torch.tensor(
                d_instruction_lens)
            ...
            for i, lengths in enumerate(g_instruction_lens):
                ...
                for j, l in enumerate(lengths):
                    ...
                    if (j % 2 == 0) or self.prefixlm:
                        features["generative"]["labels"][i,
                                                         cur_len:cur_len+l] = -100
                    cur_len += l
...
class CustomRandomSampler(torch.utils.data.sampler.RandomSampler):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        ds_indices = [torch.randperm(
            n, generator=generator).tolist() for n in self.ds_lens]
        # Increase the indices to be indices of the concatenated dataset
        ds_indices = [[i + sum(self.ds_lens[:j]) for i in ds_indices[j]]
                      for j in range(len(self.ds_lens))]
        # Create batches with only samples from one dataset
        ds_batches = [list(torch.split(torch.tensor(
            ds_indices[j]), self.total_batch_size)) for j in range(len(self.ds_lens))]
        ...
        if incomplete_indices:
            ...
            order = torch.randperm(len(incomplete_indices),
                                   generator=generator).tolist()
            incomplete_indices = torch.cat(
                [incomplete_indices[i] for i in order])
            # Then split again into groups of four & drop the last one if it is incomplete
            mixed_batches = list(torch.split(torch.tensor(
                incomplete_indices), self.total_batch_size))
        ...
        ds_batches = [int(i) for i in torch.cat([ds_batches[i]
