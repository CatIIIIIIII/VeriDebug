...
from .modeling_llama_gritlm import (
    LlamaGritConfig, LlamaGritModel, LlamaForCausalLMGrit
)
...
class GritLM(torch.nn.Module):
    def __init__(
        ...
        # One of ['cls', 'lasttoken', 'mean', 'weightedmean']
        ...
        # Passed to the model, e.g. `attn_implementation`, `torch_dtype` etc.
        **kwargs,
    ) -> None:
        super().__init__()
        self.do_classify = kwargs.pop(
            "do_classify") if "do_classify" in kwargs else False
        self.num_bug_classes = kwargs.pop(
            "num_bug_classes") if "num_bug_classes" in kwargs else None
        ...
        if self.do_classify:
            self.cls_head = torch.nn.Linear(
                self.model.config.hidden_size, self.num_bug_classes, bias=False, dtype=self.model.dtype)
    ...
    def encode(
        ...
        **kwargs,
        ...
        for start_index in tqdm(range(0, len(sentences), batch_size), desc="Batches", disable=len(sentences) < 256):
            ...
            outputs = (
                getattr(
                    self.model, self.embedding_attr) if self.embedding_attr else self.model
            )(**inputs)
            ...
            if get_cache:
                ...
                assert len(
                    all_kv_caches) == 0, "Can only get cache for one batch at a time"
        ...
        all_embeddings = (
            torch.cat(all_embeddings, dim=0) if convert_to_tensor else np.concatenate(
    ...
    def pooling(
        ...
        if self.pooling_method == 'cls':
            ...
            input_mask_expanded = attention_mask.unsqueeze(
                -1).expand((b, n, d)).float()
            embedding = torch.gather(
                hidden_state * input_mask_expanded, 1, gather_indices).squeeze(dim=1)
            ...
            s = torch.sum(
                hidden_state * attention_mask.unsqueeze(-1).float(), dim=1)
