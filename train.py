...
from src.modeling_llama_gritlm import (
    LlamaGritConfig, 
    LlamaGritModel, 
    LlamaForCausalLMGrit
)
from src.arguments import CustomTrainingArguments, DataArguments, ModelArguments
from src.data import CustomCollator, CustomDataset, CustomRandomSampler
from src.model import GritLMTrainModel

AutoConfig.register("llama_grit", LlamaGritConfig)
AutoModel.register(LlamaGritConfig, LlamaGritModel)
AutoModelForCausalLM.register(LlamaGritConfig, LlamaForCausalLMGrit)
    ...
    def filter_fn(example):
        ...
        for ex in example["pos"] + example["neg"]:
            ...
            if len(tokenizer.tokenize(BASE_BOS + USER_BOS + ex[0].strip("\t\n :") + USER_EOS + EMBED_BOS)) >= passage_max_len:
                return False
        return True
    num_proc = max(multiprocessing.cpu_count()-2,
                   1) if len(dataset) > 5000 else 1
...
def main():
    ...
    if ((training_args.gradient_accumulation_steps > 1) and
        ...
        raise ValueError(
            "Cannot use no_gen_gas or no_emb_gas without GradCache")
    ...
    for file in data_files:
        ...
        if tmp_ds_len > data_args.max_example_num_per_dataset:
            tmp_ds = tmp_ds.select(
                random.sample(list(range(tmp_ds_len)),
                              data_args.max_example_num_per_dataset)
            )
        ...
        if training_args.mode in ["embedding", "unified"] and "query" in tmp_ds.features:
            if isinstance(tmp_ds[0]['query'], (tuple, list)):
                logger.info(
                    f"Filtering out embedding samples with too long instructions for {file}")
                ...
                if num_samples:
                    assert file.split(
                        "/")[-1] in num_samples, f'Missing num_samples for {file.split("/")[-1]}'
                    tmp_ds_len = len(tmp_ds)
                    samples = num_samples[file.split("/")[-1]]
                    if tmp_ds_len > samples:
                        tmp_ds = tmp_ds.select(random.sample(
                            list(range(tmp_ds_len)), samples))
        ...
        if training_args.mode in ["unified", "generative"] and "text" in tmp_ds.features:
            if isinstance(tmp_ds[0]['text'], (tuple, list)):
                logger.info(
                    f"Filtering out generative samples with too long instructions for {file}")
                # Use passage_max_len, as this is the seq len limit for the entire generative snippet
                num_proc = max(multiprocessing.cpu_count()-2,
                               1) if tmp_ds_len > 5000 else 1
            ...
            continue
        logger.info(
            "Skipping dataset %s as its type could not be identified", file)
    ...
    if training_args.per_device_generative_bs is not None:
        ...
        logger.info("Using generative batch size %d per device",
                    training_args.per_device_generative_bs)
    ...
    train_dataset = CustomDataset(
        ...
        max_seq_len=max(data_args.query_max_len,
                        data_args.passage_max_len, data_args.generative_max_len),
    )
    ...
    if gc_chunk_size is not None:
        from src.gradcache_trainer import GradCacheTrainer
    ...
    if len(ds_embedding_lens) > 1:
        ...
        total_bs = training_args.per_device_train_batch_size * \
            training_args.gradient_accumulation_steps
